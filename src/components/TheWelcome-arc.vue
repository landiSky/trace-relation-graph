...
//     const { startPoint, endPoint } = cfg;
//     return [{
//       x: (startPoint.x + endPoint.x) / 2 + (endPoint.y - startPoint.y) * 0.2,
//       y: (startPoint.y + endPoint.y) / 2 + (startPoint.x - endPoint.x) * 0.2
//     }];
//   }
// });
...
//   draw(cfg, group) {
//     const { startPoint, endPoint } = cfg;
//     const isSameX = Math.abs(startPoint.x - endPoint.x) < 5;
    
//     if (isSameX) {
//       // 计算弧线控制点
//       const midY = (startPoint.y + endPoint.y) / 2;
//       const offsetX = Math.abs(startPoint.y - endPoint.y) * 0.3;
//       const path = [
//         ['M', startPoint.x, startPoint.y],
//         ['Q', startPoint.x + offsetX, midY, endPoint.x, endPoint.y]
//       ];
...
//     // 判断是否双向边
//     const isBidirectional = graph.value.getEdges().some(edge => 
//       edge.getSource() === targetNode && edge.getTarget() === sourceNode
//     );
    
//     // 判断是否同层级（Y轴坐标相近）
//     const isSameLevel = Math.abs(sourceNode.getModel().y - targetNode.getModel().y) < 50;
    
//     if (isBidirectional || isSameLevel) {
//       // 绘制弧线（带自动避让）
//       const midPoint = {
...
//           ],
//           stroke: '#1890FF',
//           endArrow: G6.Arrow.triangle(10, 12, 25)
//         }
//       });
//     } else {
//       // 绘制直线
...
//           y2: endPoint.y,
//           stroke: '#A3B1BF',
//           endArrow: G6.Arrow.triangle(8, 10, 20)
//         }
//       });
//     }
//   }

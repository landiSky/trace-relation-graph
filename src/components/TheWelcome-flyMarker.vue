...
G6.registerEdge(
  ...
  {
    afterDraw(cfg, group) {
      console.log("afterDraw", cfg, group, activeEdges.value);
      ...
      // 清除旧小球
      group
        .findAll((ele) => ele.get("name") === "circle-shape")
        .forEach((b) => b.remove());
...
const initGraph = () => {
  ...
  graph.value.on("node:click", (e) => {
    console.log('click', e, e.item.getModel())
    const nodeId = e.item.getModel().id;
    clearAllAnimations();
...
const handleNodeClick = (e) => {
  ...
  // edges.forEach((edge) => {
  //   const group = edge.getContainer();
  //   const circle = group.find((ele) => ele.get("name") === "circle-shape");
  //   circle?.stopAnimate();
  //   group.removeChild(circle, true);
  // });

  const nodeId = node.getModel().id;

  // // 重新触发关联边动画
  // const relatedEdges = graph.value.getRelatedEdges(node.getID());
  // const relatedEdges = graph.value.getEdges().filter((edge) => {
  ...
  // activeEdges.value = new Set(relatedEdges.map((e) => e.getID()));
   // 节点点击事件处理

     // 重置所有边状态
  graph.value.getEdges().forEach(edge => {
    graph.value.clearItemStates(edge, 'active')
  })

  // 激活与当前节点相连的边
  graph.value.getEdges().forEach(edge => {
    if (edge.getSource().getID() === nodeId || 
        edge.getTarget().getID() === nodeId) {
      graph.value.setItemState(edge, 'active', true)
      graph.value.refreshItem(edge) // 强制重绘触发动画
    }
  })
  //  initGraph();

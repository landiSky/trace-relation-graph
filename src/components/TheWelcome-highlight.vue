...
// 注册自定义边类型
G6.registerEdge('flow-highlight', {
  draw(cfg, group) {
    ...
    // 添加流动动画层
    const flow = group.addShape('path', {
      attrs: {
        path,
        stroke: '#1890ff',
        lineWidth: 0,
  ...
  afterDraw(cfg, group) {
    const flow = group.get('children')[1];
    let offset = 0;
...
const initGraph = () => {
  ...
  graph.value.on("node:click", (e) => {
    ...
    // 高亮当前节点
    highlightCache.nodes.add(model.id);
    ...
    graph.value.getEdges().forEach(edge => {
      ...
      if (edgeModel.source === model.id) {
        highlightCache.edges.add(edgeModel.id);
        highlightCache.nodes.add(edgeModel.target);
        ...
        graph.value.updateItem(targetNode, {
          style: {
            fill: '#69c0ff',
            stroke: '#096dd9'
          }
...
// 贝塞尔曲线点计算
function getPointOnQuadraticBezier(p0x, p0y, p1x, p1y, p2x, p2y, t) {
  const mt = 1 - t;
  return {
...
function getControlPoint(start, end) {
  ...
  // 动态计算偏移量，保持弧度一致性
  const offset = Math.hypot(end.x - start.x, end.y - start.y) * 0.3;
  return {
    x: midX + (end.y - start.y) * 0.15,
    y: midY - (end.x - start.x) * 0.15
  };
}

// 更新动画路径计算
function createFlowAnimation(edge) {
  const model = edge.getModel();
  const { startPoint, endPoint } = model;
  const ctrlPt = getControlPoint(startPoint, endPoint);
  const edgeGroup = edge.getContainer();

  
  edgeGroup.addShape('circle', {
    attrs: {
      x: startPoint.x,
      y: startPoint.y,
      r: 6,
    ...
    animate: {
      onFrame(ratio) {
        const x = (1-ratio)**2 * startPoint.x + 
                 2*(1-ratio)*ratio * ctrlPt.x + 
                 ratio**2 * endPoint.x;
        const y = (1-ratio)**2 * startPoint.y + 
                 2*(1-ratio)*ratio * ctrlPt.y + 
                 ratio**2 * endPoint.y;
        return { x, y };
      },
      duration: 1500,
...
function clearHighlights() {
  highlightCache.nodes.forEach(id => {
  ...
  highlightCache.edges.forEach(id => {
    graph.value.updateItem(id, {
      style: {
        stroke: '#aaa',
        lineWidth: 2
  ...
  highlightCache.nodes.clear();
  highlightCache.edges.clear();
}
